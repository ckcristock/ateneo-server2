...
class WaitingListService
{
    ...
    static public function index()
    {
        ...
        return DB::table('waiting_lists')
            ...
            ->when(request()->get('date'), fn ($q, $date) => $q->whereDate('waiting_lists.created_at', $date))
            ->when(request()->get('patient') && request()->get('patient') != 'null', fn ($q) => $q->where(function ($query) {
                $query->where('patients.identifier', request()->get('patient'))
                    ->orWhereRaw("CONCAT(`firstname`, ' ', `surname`) LIKE ?", ['%' . request()->get('patient') . '%']);
            }))
            ->when(request()->get('speciality') && request()->get('speciality') != 'null', fn ($q) => $q->where('waiting_lists.speciality_id', request()->get('speciality')))
            ->when(request()->get('institution') && request()->get('institution') != 'null', fn ($q) => $q->where('companies.id', request()->get('institution')))
            ->whereNull('appointments.space_id')
            ->where('waiting_lists.state', '=', 'Pendiente')
            ->select(
                'waiting_lists.*',
