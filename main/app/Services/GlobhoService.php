...
class GlobhoService
{
    ...
    public static function sendGlobho($appointment)
    {
        ...
        try {
            ...
            if (!$appointment->space)
                throw new Exception("Cita Sin spacio asignado " . $appointment->id, 400);
            if (!$appointment->callin->patient)
                throw new Exception("Cita Sin paciente " . $appointment->id, 400);
            ...
            if (!$cup)
                throw new Exception("Cita Sin cup service " . $appointment->id, 400);
            if (!$location)
                throw new Exception("Cita Sin sede " . $appointment->id, 400);
            if (!$contract)
                throw new Exception("Cita Sin contrato " . $appointment->id, 400);
            if (!$typeDocument)
                throw new Exception("Cita Sin tipo de documento " . $appointment->id, 400);
            if (!$regimenType)
                throw new Exception("Cita Sin regimen " . $appointment->id, 400);
            if (!$municipality)
                throw new Exception("Cita Sin ciudad " . $appointment->id, 400);
            if (!$department)
                throw new Exception("Cita Sin departamento " . $appointment->id, 400);
            if (!$company)
                throw new Exception("Cita Sin ips del paciente  " . $appointment->id, 400);
