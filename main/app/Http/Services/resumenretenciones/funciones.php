<?php
namespace App\Http\Services\resumenretenciones;
...
class funciones
{
    public static function listaCartera($nit, $id_plan_cuenta, $fecha = false)
    {
        $condicion_cuenta = '';
        if ($id_plan_cuenta) {
    ...
    public static function cambiarEstadoFactura($nit, $documento, $id_plan_cuenta)
    {
    ...
    public static function anularDocumento($datos)
    {
        $tipo = $datos['Tipo'];
            ...
            switch ($tipo) {
                case 'Egreso':
                    ...
                    if ($contabilidad->validarMesOrAnioCerrado($fecha)) {
                        ...
                        self::registrarActividad('Anulacion', $datos);
                    } else {
                        $anulacion = false;
                    }
    ...
    private static function registrarActividad($tipo_registro, $datos)
    {
    ...
    function getListaTerceros($tipo = null, $condiciones = '', $limit = '')
    {
        ...
        $query = "SELECT r.* FROM (
            (SELECT Id_Proveedor AS Nit, Rut, Digito_Verificacion, Tipo AS Tipo_Persona, Primer_Nombre, Segundo_Nombre, Primer_Apellido, Segundo_Apellido, Razon_Social, Nombre AS Nombre_Comercial, Direccion, Telefono AS Telefono_Fijo, Celular AS Telefono_Celular, Correo, (SELECT name FROM departments WHERE id = P.Id_Departamento) AS Departamento, '' AS Zona, (SELECT name FROM municipalities WHERE id = P.Id_Municipio) AS Municipio, Regimen AS Tipo_Regimen, Tipo_Retencion, Animo_Lucro, Ley_1429_2010, (SELECT Descripcion FROM ciiu_codes WHERE value = P.Id_Codigo_Ciiu) AS Actividad_Economica, Tipo_Reteica, Contribuyente AS Gran_Contribuyente, IF(Condicion_Pago IN (0,1), 'Contado', CONCAT(Condicion_Pago,' Días')) AS Plazo, Estado, Tipo_Tercero FROM Proveedor P)
            UNION
            (SELECT Id_Cliente AS Nit,Rut, Digito_Verificacion, Tipo AS Tipo_Persona, Primer_Nombre, Segundo_Nombre, Primer_Apellido, Segundo_Apellido, Razon_Social, Nombre AS Nombre_Comercial, Direccion, Telefono_Persona_Contacto AS Telefono_Fijo, Celular AS Telefono_Celular, Correo_Persona_Contacto AS Correo, (SELECT name FROM departments WHERE id = P.Id_Departamento) AS Departamento, (SELECT name FROM zones WHERE id = P.Id_Zona) AS Zona, (SELECT name FROM municipalities WHERE id = P.Id_Municipio) AS Municipio, Regimen AS Tipo_Regimen, '' AS Tipo_Retencion, Animo_Lucro, '' AS Ley_1429_2010, (SELECT Descripcion FROM ciiu_codes WHERE value = P.Id_Codigo_Ciiu) AS Actividad_Economica, Tipo_Reteica, Contribuyente AS Gran_Contribuyente, IF(Condicion_Pago IN (0,1), 'Contado', CONCAT(Condicion_Pago,' Días')) AS Plazo, Estado, 'Cliente' AS Tipo_Tercero FROM Cliente P)
            UNION
    ...
    public static function generarConsecutivoTipoActivo($id)
    {
    ...
    public static function eliminarBorradorContable($id)
    {
